...
async function permission(tx,actorId,kind){
...
export async function listPersonnelFinance(prisma,actorId,params=new URLSearchParams()){
 const kind=params.get('kind')||'EMPLOYMENT',access=await permission(prisma,actorId,kind)
...
export async function savePersonnelFinance(prisma,actorId,input){
 ...
 return transaction(prisma,async tx=>{
  const access=await permission(tx,actorId,input.kind);if(!access.edit)fail('PERMISSION_DENIED',403)
...
export async function commandPersonnelFinance(prisma,actorId,input){
 ...
 return transaction(prisma,async tx=>{
  ...
  const access=await permission(tx,actorId,old.kind)
