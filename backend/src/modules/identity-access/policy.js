...
export function grantsFor(role) {
  ...
  if (role === 'MANAGER') return ['profile.read', 'users.read', 'users.invite', 'users.roles', 'users.suspend', 'audit.read']
  return ['profile.read']
}
...
export function publicProfile(profile, email) {
  return { id: profile.id, email, displayName: profile.displayName, status: profile.status,
