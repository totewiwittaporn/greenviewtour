...
export async function listUsers(pool, { search = '', page = 1, pageSize = 25, department = null } = {}) {
  ...
  try {
    ...
    const { rows } = await client.query(`SELECT u.id, email, email_confirmed_at, created_at, last_sign_in_at, p."displayName", p.nickname, p.department, p.status, p."updatedAt", p.address, p."primaryPhone", p."emergencyPhone", p."lineId", ${addressKeys.map(key=>`p."${key}"`).join(', ')},
