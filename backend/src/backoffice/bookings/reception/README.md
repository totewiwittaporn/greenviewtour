...
Own this page’s request validation, transport handlers and response composition here when implemented. Delegate domain rules and writes to `backend/src/modules/bookings`, `backend/src/modules/operations`, `backend/src/modules/finance`. Do not create parallel domain models, direct database access or duplicate services here.
