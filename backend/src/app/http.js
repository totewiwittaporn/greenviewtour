...
import { getBlueprint, saveBooking, bookingStatus, tripPreparation, amendBookingDetails } from '../modules/operations/bookings.js'
  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (operationMatch) {
        ...
        if (req.method === 'GET') return send(200, entity === 'daily-summary' ? await dailySummaryState(prisma,user.id,url.searchParams.get('date'),process.env,Number(url.searchParams.get('page')||1)) : entity === 'jobs' ? await listJobs(prisma,user.id,url.searchParams) : entity === 'dispatch-options' ? await dispatchOptions(prisma,user.id,url.searchParams) : entity === 'booking-options' ? await bookingOptions(prisma,user.id,url.searchParams) : entity === 'preparation' ? await tripPreparation(prisma,user.id,url.searchParams) : entity === 'blueprint' ? await getBlueprint(prisma,user.id,url.searchParams) : await listOperations(prisma,user.id,entity,url.searchParams))
        const input = await body(req,131072)
        return send(200, entity === 'daily-summary' ? await prepareDailySummary(prisma,user.id,input) : entity === 'runs' ? await saveRun(prisma,user.id,input) : entity === 'dispatch-command' ? await dispatchCommand(prisma,user.id,input) : entity === 'stock-command' ? await stockCommand(prisma,user.id,input) : entity === 'booking-details' ? await amendBookingDetails(prisma,user.id,input) : entity === 'booking-status' ? await bookingStatus(prisma,user.id,input) : entity === 'bookings' ? await saveBooking(prisma,user.id,input) : await saveOperationCatalog(prisma,user.id,entity,input))
      }
