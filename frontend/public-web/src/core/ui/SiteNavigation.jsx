...
export function SiteHeader(){const {t,label}=useLocale();return <header className="site-header"><a href="/" aria-label={t("Greenview Tour หน้าแรก")}><img className="logo" src={`${base}/wp-content/uploads/2024/12/greenview-tour-logo-1.png`} alt="Greenview Tour"/></a><LanguageSelector/><nav aria-label={t("เมนูหลัก")}><a href="/tours">{label("โปรแกรมทัวร์")}</a><a href="/promotions">{label("โปรโมชั่น")}</a><a href="/#surin">{label("รู้จักเกาะสุรินทร์")}</a><a href="/#contact">{label("ติดต่อเรา")}</a></nav><a className="public-button customer-login" href={customerLogin}>{label("เข้าสู่ระบบลูกค้า")}</a></header>}
