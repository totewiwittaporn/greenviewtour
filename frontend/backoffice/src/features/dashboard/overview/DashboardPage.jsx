...
export default function DashboardPage() {
 ...
 return <div className="dashboard"><div className="page-heading"><div><span className="eyebrow">{bilingualLabel(data?.scope || 'YOUR WORKSPACE')}</span><h1 tabIndex="-1">{bilingualLabel("Dashboard")}</h1><p>{t("Your work, upcoming customers and items needing attention.")}</p></div><RefreshButton onClick={reload} disabled={state.loading}/></div>
 ...
 {data.calendar && <section className="panel dashboard-calendar" aria-labelledby="customer-calendar-title"><h2 id="customer-calendar-title">{bilingualLabel("Customers · next 14 days")}</h2><p>{t("Confirmed and completed bookings, counted once on arrival. Return-only bookings use their return service date.")}</p><div className="dashboard-days">{data.calendar.map(day => <button type="button" key={day.date} className="dashboard-day" aria-label={t("{value0}, {value1} guests, {value2} bookings", {value0: dateLabel(day.date), value1: number(day.pax), value2: number(day.bookings)})} onClick={() => setSelected(day)}><time dateTime={day.date}>{dateLabel(day.date)}</time><strong>{number(day.pax)}</strong><span>{t("guests")}</span><small>{number(day.bookings)}{' '}{t("bookings")}</small></button>)}</div></section>}
 <section aria-labelledby="attention-title"><h2 id="attention-title">{bilingualLabel("Work needing attention")}</h2><p>{t("Counts follow each work area's permissions. Overdue uses a recorded due date or scheduled end time.")}</p>{data.widgets.length ? <div className="dashboard-widgets">{data.widgets.map(widget => <AttentionWidget key={widget.id} widget={widget}/>)}</div> : <section className="panel dashboard-state"><h3>{bilingualLabel("No work areas available yet")}</h3><p>{t("Your account has no supported work permissions. Contact your Manager to check your assignments.")}</p><a href="/profile">{t("Open my profile →")}</a></section>}</section>
 </>}
