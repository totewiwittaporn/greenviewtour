...
export default function ProfilePage({ user, onSaved }) {
  ...
  return <>
    {reloadConfirm&&<Dialog title="Reload profile?" onClose={()=>setReloadConfirm(false)}><p>Discard your unsaved profile changes and reload?</p><div className="dialog-actions"><Button autoFocus onClick={()=>setReloadConfirm(false)}>Keep editing</Button><Button onClick={()=>reload(true)}>Discard and reload</Button></div></Dialog>}
