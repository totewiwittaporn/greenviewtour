...
export default function ProfilePage({ user, onSaved }) {
 useLocale();
  ...
  if (done) return <section className="panel auth-result" role="status"><h1>{t("Password changed")}</h1><p>{t("Sign in again with your new password.")}</p>{done.warning && <p>{t("Your password was changed, but some session cleanup could not finish. Contact your Manager if other devices remain signed in.")}</p>}<a className="button button-primary" href="/login">{t("Return to sign in")}</a></section>
  return <>
    {reloadConfirm&&<Dialog title={t("Reload profile?")} onClose={()=>setReloadConfirm(false)}><p>{t("Discard your unsaved profile changes and reload?")}</p><div className="dialog-actions"><Button autoFocus onClick={()=>setReloadConfirm(false)}>{t("Keep editing")}</Button><Button onClick={()=>reload(true)}>{t("Discard and reload")}</Button></div></Dialog>}
    <section className="page-heading"><div><p className="eyebrow">{t("YOUR ACCOUNT")}</p><h1>{t("Edit profile")}</h1><p className="muted">{t("Keep your details up to date so your team can reach you.")}</p></div><a className="button" href="/">{t("Back to workspace")}</a></section>
    <div className="profile-layout"><aside className="panel profile-summary"><div className="profile-identity"><span className="profile-avatar">{saved.displayName?.[0]?.toUpperCase()}</span><div><h2>{saved.displayName}</h2><p>{saved.email}</p></div></div><dl className="profile-details"><dt>{t("Department")}</dt><dd>{t(saved.department || 'Not assigned')}</dd><dt>{t("Roles")}</dt><dd>{saved.roles.map(role => t(role.name)).join(' · ')}</dd><dt>{t("Status")}</dt><dd>{t(saved.status)}</dd><dt>{t("Joined")}</dt><dd>{saved.createdAt ? displayDate(new Date(saved.createdAt), { dateStyle: 'medium', timeZone: 'Asia/Bangkok' }) : t('Not available')}</dd></dl><p className="muted">{t("Contact your Manager to update your account access.")}</p></aside>
      <div className="profile-sections"><section className="panel"><div className="panel-heading"><div><h2>{t("Personal & contact details")}</h2><p>{t("Contact details are optional and visible to authorized team managers.")}</p></div></div><form className="profile-form" ref={form} noValidate onSubmit={save} aria-busy={busy}>
        <FormField label={t("Display name")} value={fields.displayName} onChange={e => update('displayName', e.target.value)} maxLength={100} autoComplete="name" error={errors.displayName} disabled={busy} />
        ...
        <FormField label={t("Line ID")} value={fields.lineId} onChange={e => update('lineId', e.target.value)} maxLength={100} autoComplete="off" disabled={busy} />
      ...
      <section className="panel"><div className="panel-heading"><div><h2>{t("Change password")}</h2><p>{t("Confirm your current password. You will sign in again after the change.")}</p></div></div><form className="profile-form" ref={passwordForm} noValidate onSubmit={changePassword} aria-busy={busy}>
