...
export default function AuthPage({ mode = 'login' }) {
 const { locale } = useLocale()
  ...
  useEffect(() => { document.title = `${t(title)} · Greenview Tour` }, [title, locale])
  useEffect(() => {
    if (mode === 'register') {
  ...
  return <AuthLayout title={t(title)} description={t(description)}>
    ...
    <div className="auth-switch">{mode === 'login' ? <>{t("Need access? Ask your Manager for an invitation.")}</> : <a href="/login">{t("Back to sign in")}</a>}</div>
  </AuthLayout>
}
