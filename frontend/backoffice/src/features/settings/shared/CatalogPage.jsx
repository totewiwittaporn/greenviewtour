...
import WebsiteImageUpload from './WebsiteImageUpload.jsx'
...
const messages={PROMOTION_PRICE_INVALID:'A promotional fare cannot exceed the regular tour fare.',PROMOTION_IN_USE:'This promotion has requests. Create a new promotion round to change its price, dates or quota.',AGREEMENT_AGENT_MISMATCH:'Choose an agreement belonging to this agent.',AGENT_RATE_PERIOD_OVERLAP:'An active price already covers this period for this agent and program. Retire an undated price or use a non-overlapping agreement.',AGREEMENT_IN_USE:'This agreement has prices. Create a new annual agreement to change its agent or effective period. Retire its prices before changing agreement availability.',SETTINGS_CONFLICT:'This record changed. Close this form and refresh the list before editing again.',SETTINGS_DUPLICATE:'This code or agent–tour price already exists. Edit the existing record.',RELATED_RECORD_UNAVAILABLE:'A selected partner or tour is no longer active or has a different role. Choose an available record.',PARTNER_IN_USE:'This partner is used by active tours, prices or vehicles. Update those records before removing its role or deactivating it.',TOUR_IN_USE:'Deactivate this tour’s agent prices before deactivating the tour.',PERMISSION_DENIED:'Your account cannot manage company settings.',INVALID_SETTINGS:'Check the fields and try again.'}
...
function Related({field,value,label,onChange,error,disabled}){
 ...
 return <ReferenceField label={field.label} value={value} selectedLabel={label} onChange={onChange} error={t(error)} load={load} disabled={disabled}/>
}
function EditorSurface({inline,children,...props}) {
 useLocale(); return inline?<section className="panel">{children}</section>:<Dialog {...props}>{children}</Dialog> }
function Editor({entity,row,onClose,onSaved,readOnly=false,inline=false}){
 ...
 return <EditorSurface inline={inline} title={`${bilingualLabel(readOnly?'View':row?'Edit':'Add')} ${bilingualLabel(definition.singular)}`} onClose={close} busy={busy}>
 {discard?<section><p>{t("Discard your unsaved changes?")}</p><div className="dialog-actions"><Button autoFocus onClick={()=>setDiscard(false)}>{t("Keep editing")}</Button><Button onClick={onClose}>{t("Discard changes")}</Button></div></section>:readOnly?<dl className="catalog-details">{definition.fields.filter(f=>!f.hidden&&visibleField(f,values)).map(f=><div key={f.key}><dt>{f.label}</dt><dd>{f.type==='money'?money(row[f.key]):f.type==='roles'?values[f.key].map(labelFor).join(', '):f.type==='select'?t(labelFor(values[f.key])):f.type==='reference'?(row[relation(f.key)]?.name||t("Not set")):f.key==='mapUrl'&&mapLinks(row).pin?<a href={mapLinks(row).pin} target="_blank" rel="noopener noreferrer">{t("Open saved pin")}</a>:values[f.key]||t("Not set")}</dd></div>)}</dl>:<form className={inline?'profile-form company-form':undefined} ref={form} noValidate onSubmit={save}>
 {definition.operation&&<p>{operationHints[entity]}</p>}
 {entity==='agreements'&&<p>{t("Keep one record for each annual agreement. Attach an HTTPS reference to the signed evidence. Renewals use a new record so previous agreement periods remain available.")}</p>}
 {entity==='rates'&&<p>{t("Prices charged by Greenview to this agent, per passenger. These do not change direct customer prices. Leave an unavailable passenger price empty.")}</p>}
 {entity==='tours'&&<p>{t("Prices are per passenger in THB. Empty means not set; zero is an explicit free price. Booking and Public publishing are configured separately.")}</p>}
 {['tours','popups'].includes(entity)&&<WebsiteImageUpload disabled={busy} onUploaded={url=>update(entity==='tours'?'imageUrls':'imageUrl',entity==='tours'?[values.imageUrls,url].filter(Boolean).join('\n'):url)}/>}
 ...
 {entity==='popups'&&values.imageUrl&&<section className="address-section" aria-label={t("Pop-up preview")}><h3>{bilingualLabel("Pop-up preview")}</h3><img style={{maxWidth:'100%',maxHeight:400,objectFit:'contain'}} src={values.imageUrl.replace('/api/public/images/','/api/website-images/')} alt={values.imageAlt||'Pop-up image'}/><p>{values.title}</p>{values.linkUrl&&<p>{t("Destination:")}{' '}{values.linkUrl}</p>}<p>{values.status==='ACTIVE'?t('Displays on the website during the selected dates.'):t('Draft: not shown on the website.')}</p></section>}
 {notice&&<div role="alert"><p>{t(notice)}</p>{inline&&<Button disabled={busy} onClick={close}>{t("Reload company details")}</Button>}</div>}{confirm&&<p role="alert">{t("This changes availability or partner roles. Existing references will be checked before saving. Confirm to continue.")}</p>}
 <div className="dialog-actions"><Button type="submit" className={inline?'button-primary':undefined} busy={busy} disabled={busy}>{confirm?t('Confirm changes'):inline?t('Save company details'):t('Save changes')}</Button></div><span className="sr-only" role="status">{busy?t('Saving changes…'):''}</span>
 </form>}
 </EditorSurface>
}
export default function CatalogPage({entity}){
 ...
 function detail(row){if(['seasons','promotions'].includes(entity))return `${row.tour?.name||'Tour'} · ${String(row.startsOn).slice(0,10)} – ${String(row.endsOn).slice(0,10)}`;if(entity==='popups')return `${String(row.startsOn).slice(0,10)} – ${String(row.endsOn).slice(0,10)}`;if(definition.operation)return entity==='components'?`${row.tour?.name||'Tour'} · ${t(labelFor(row.selection))} · ${row.quantity} ${t(labelFor(row.basis))}`:entity==='slots'?`${row.resource?.name||'Service'} · ${row.capacity} units`:entity==='trips'?`${row.tour?.name||'Standalone service'} · ${row.capacity} passengers`:entity==='stores'?t(labelFor(row.kind)):`${t(labelFor(row.category))} · ${t(labelFor(row.baseUnit))}${row.size?` · ${row.size}`:''}`;return entity==='channels'?t(labelFor(row.kind)):entity==='partners'?row.roles.map(labelFor).join(' · '):entity==='tours'?(row.operator?.name||'Greenview Tour'):entity==='vehicles'?`${t(labelFor(row.kind))} · ${row.capacity} passengers`:entity==='locations'?`${t(labelFor(row.kind))}${row.zone?` · ${row.zone}`:''}`:row.email||row.phone||'Not set'}
 if(entity==='company')return <><div className="page-heading"><div><span className="eyebrow">{bilingualLabel("COMPANY SETTINGS")}</span><h1 tabIndex={-1}>{bilingualLabel("Company")}</h1><p>{t("Your company details. One shared record for Greenview Tour.")}</p></div></div>{notice&&<p role="status">{t(notice)}</p>}{state.loading?<section className="panel auth-result"><p role="status">{t("Loading company details…")}</p></section>:state.error?<section className="panel auth-result"><p role="alert">{t(state.error)}</p><Button onClick={()=>setAttempt(n=>n+1)}>{t("Retry")}</Button></section>:<Editor key={state.rows[0]?.version||0} entity="company" row={state.rows[0]} inline onClose={()=>setAttempt(n=>n+1)} onSaved={()=>{setNotice('Company details saved.');setAttempt(n=>n+1)}}/>}</>
 return <><div className="page-heading"><div><span className="eyebrow">{definition.operation?bilingualLabel('TOUR OPERATIONS'):bilingualLabel('COMPANY SETTINGS')}</span><h1 tabIndex={-1}>{bilingualLabel(definition.title)}</h1><p>{definition.operation?operationHints[entity]:entity==='rates'?t('Set a separate price for each agent and tour program.'):t('Manage the information your team uses for tour bookings.')}</p></div><Button disabled={state.loading||Boolean(state.error)||(entity==='company'&&state.rows.length>0)} onClick={()=>setEditor({})}>{t("+ Add")}{' '}{t(definition.singular)}</Button></div>
 <SummaryCards label={bilingualLabel("{value0} summary — all records, independent of filters", {value0: bilingualLabel(definition.title)})} items={[
 ...
 <section className="panel table-panel"><div className="panel-heading"><div><h2>{bilingualLabel(definition.title)}</h2><p>{t("Search records and open Actions to view or edit.")}</p></div></div><div className="filterbar">{entity!=='company'&&<><SearchField label={bilingualLabel("Search {value0}", {value0: bilingualLabel(definition.title.toLowerCase())})} placeholder={isRates?t('Search agent or tour…'):t('Search by name or code…')} value={query} onChange={q=>{setQuery(q);setPage(1)}} onCompositionChange={setComposing}/><SelectField label={bilingualLabel("Status filter")} value={status} onChange={e=>{setStatus(e.target.value);setPage(1)}}><option value="">{t("All statuses")}</option>{entity==='trips'?<option value="OPEN">{t("Open")}</option>:<><option value="ACTIVE">{t("Active")}</option><option value="INACTIVE">{t("Inactive")}</option></>}</SelectField></>}<RefreshButton disabled={state.loading} onClick={()=>setAttempt(n=>n+1)}/></div>
 {notice&&<p className="catalog-notice" role="status">{t(notice)}</p>}
 <DataTable columns={columns} busy={state.loading} label={bilingualLabel("{value0} table", {value0: bilingualLabel(definition.title)})} error={state.error} onRetry={()=>setAttempt(n=>n+1)} isEmpty={!state.rows.length} loadingLabel={`Loading ${definition.title.toLowerCase()}…`} empty={query||status?'No matching records. Clear your filters to see more.':`No ${definition.title.toLowerCase()} yet. Add your first record to get started.`}>
 {state.rows.map(row=><tr key={row.id}><td><strong>{isRates?row.agent?.name:row.name}</strong><span className="cell-sub">{isRates?row.tour?.name:row.code||row.legalName||t("Company details")}</span></td>{isRates?<><td>{money(row.adultPrice)}</td><td>{money(row.childPrice)}</td></>:<td>{detail(row)}</td>}<td>{t(labelFor(row.status))||t("Company")}</td><td><Dropdown rowActions label={bilingualLabel("Actions for {value0}", {value0: row.name||row.agent?.name})} items={[{label:'View',icon:'view',onSelect:()=>setEditor({row,readOnly:true})},{label:'Edit',icon:'edit',onSelect:()=>setEditor({row})},...(entity==='trips'?[{label:'Preparation',icon:'briefcase',onSelect:()=>setPreparation(row)}]:[]),...(mapLinks(row).pin?[{label:'Open saved pin',icon:'pin',href:mapLinks(row).pin,target:'_blank'}]:[])]}/></td></tr>)}
 </DataTable><Pagination page={state.page||page} pageSize={state.pageSize||25} total={state.error||state.loading?undefined:state.total} busy={state.loading} onPageChange={setPage} label={bilingualLabel("{value0} pagination", {value0: bilingualLabel(definition.title)})}/></section>
