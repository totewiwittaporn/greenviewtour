...
const summaryFeature={partners:['Sales agents','Partners with the sales agent role'],tours:['Greenview tours','Organized by Greenview Tour'],rates:['Child prices set','Price records with a child fare'],locations:['Hotels','Pickup locations classified as hotels'],vehicles:['Greenview fleet','Vehicles and boats provided by Greenview'],channels:['Direct channels','Sales channels with a direct source']}
...
function Editor({entity,row,onClose,onSaved,readOnly=false,inline=false}){
 ...
 {discard?<section><p>Discard your unsaved changes?</p><div className="dialog-actions"><Button autoFocus onClick={()=>setDiscard(false)}>Keep editing</Button><Button onClick={onClose}>Discard changes</Button></div></section>:readOnly?<dl className="catalog-details">{definition.fields.filter(f=>!f.hidden&&visibleField(f,values)).map(f=><div key={f.key}><dt>{f.label}</dt><dd>{f.type==='money'?money(row[f.key]):f.type==='roles'?values[f.key].map(labelFor).join(', '):f.type==='select'?labelFor(values[f.key]):f.type==='reference'?(row[f.key==='agentId'?'agent':f.key==='tourId'?'tour':f.key==='operatorId'?'operator':'provider']?.name||'Not set'):f.key==='mapUrl'&&mapLinks(row).pin?<a href={mapLinks(row).pin} target="_blank" rel="noopener noreferrer">Open saved pin</a>:values[f.key]||'Not set'}</dd></div>)}</dl>:<form className={inline?'profile-form company-form':undefined} ref={form} noValidate onSubmit={save}>
...
export default function CatalogPage({entity}){
 ...
 {label:summaryFeature[entity][0],icon:'briefcase',value:state.summary?.featured??'—',detail:summaryFeature[entity][1]},
 ]}/>
