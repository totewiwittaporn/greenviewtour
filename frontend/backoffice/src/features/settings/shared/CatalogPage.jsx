...
function Editor({entity,row,onClose,onSaved,readOnly=false,inline=false}){
 ...
 return <EditorSurface inline={inline} title={`${t(readOnly?'View':row?'Edit':'Add')} ${t(definition.singular)}`} onClose={close} busy={busy}>
...
export default function CatalogPage({entity}){
 ...
 <section className="panel table-panel"><div className="panel-heading"><div><h2>{t(definition.title)}</h2><p>{t("Search records and open Actions to view or edit.")}</p></div></div><div className="filterbar">{entity!=='company'&&<><SearchField label={t("Search {value0}", {value0: t(definition.title.toLowerCase())})} placeholder={isRates?t('Search agent or tour…'):t('Search by name or code…')} value={query} onChange={q=>{setQuery(q);setPage(1)}} onCompositionChange={setComposing}/><SelectField label={t("Status filter")} value={status} onChange={e=>{setStatus(e.target.value);setPage(1)}}><option value="">{t("All statuses")}</option>{entity==='trips'?<option value="OPEN">{t("Open")}</option>:<><option value="ACTIVE">{t("Active")}</option><option value="INACTIVE">{t("Inactive")}</option></>}</SelectField></>}<RefreshButton disabled={state.loading} onClick={()=>setAttempt(n=>n+1)}/></div>
 {notice&&<p className="catalog-notice" role="status">{t(notice)}</p>}
 <DataTable columns={columns} busy={state.loading} label={t("{value0} table", {value0: t(definition.title)})} error={state.error} onRetry={()=>setAttempt(n=>n+1)} isEmpty={!state.rows.length} loadingLabel={`Loading ${definition.title.toLowerCase()}…`} empty={query||status?'No matching records. Clear your filters to see more.':`No ${definition.title.toLowerCase()} yet. Add your first record to get started.`}>
 {state.rows.map(row=><tr key={row.id}><td><strong>{isRates?row.agent?.name:row.name}</strong><span className="cell-sub">{isRates?row.tour?.name:row.code||row.legalName||t("Company details")}</span></td>{isRates?<><td>{money(row.adultPrice)}</td><td>{money(row.childPrice)}</td></>:<td>{detail(row)}</td>}<td>{t(labelFor(row.status))||t("Company")}</td><td><Dropdown rowActions label={t("Actions for {value0}", {value0: row.name||row.agent?.name})} items={[{label:'View',icon:'view',onSelect:()=>setEditor({row,readOnly:true})},{label:'Edit',icon:'edit',onSelect:()=>setEditor({row})},...(entity==='trips'?[{label:'Preparation',icon:'briefcase',onSelect:()=>setPreparation(row)}]:[]),...(mapLinks(row).pin?[{label:'Open saved pin',icon:'pin',href:mapLinks(row).pin,target:'_blank'}]:[])]}/></td></tr>)}
 </DataTable><Pagination page={state.page||page} pageSize={state.pageSize||25} total={state.error||state.loading?undefined:state.total} busy={state.loading} onPageChange={setPage} label={t("{value0} pagination", {value0: t(definition.title)})}/></section>
