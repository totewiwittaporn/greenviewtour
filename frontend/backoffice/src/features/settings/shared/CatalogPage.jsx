...
function Editor({entity,row,onClose,onSaved,readOnly=false,inline=false}){
 ...
 {entity==='agreements'&&<p>Keep one record for each annual agreement. Attach an HTTPS reference to the signed evidence. Renewals use a new record so previous agreement periods remain available.</p>}
 ...
 {entity==='popups'&&values.imageUrl&&<section className="address-section" aria-label="Pop-up preview"><h3>Pop-up preview</h3><img style={{maxWidth:'100%',maxHeight:400,objectFit:'contain'}} src={values.imageUrl.replace('/api/public/images/','/api/website-images/')} alt={values.imageAlt||'Pop-up image'}/><p>{values.title}</p>{values.linkUrl&&<p>Destination: {values.linkUrl}</p>}<p>{values.status==='ACTIVE'?'Displays on the website during the selected dates.':'Draft: not shown on the website.'}</p></section>}
...
export default function CatalogPage({entity}){
 ...
 <section className="panel table-panel"><div className="panel-heading"><div><h2>{definition.title}</h2><p>Search records and open Actions to view or edit.</p></div></div><div className="filterbar">{entity!=='company'&&<><SearchField label={`Search ${definition.title.toLowerCase()}`} placeholder={isRates?'Search agent or tour…':'Search by name or code…'} value={query} onChange={q=>{setQuery(q);setPage(1)}} onCompositionChange={setComposing}/><SelectField label="Status filter" value={status} onChange={e=>{setStatus(e.target.value);setPage(1)}}><option value="">All statuses</option>{entity==='trips'?<option value="OPEN">Open</option>:<><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></>}</SelectField></>}<RefreshButton disabled={state.loading} onClick={()=>setAttempt(n=>n+1)}/></div>
