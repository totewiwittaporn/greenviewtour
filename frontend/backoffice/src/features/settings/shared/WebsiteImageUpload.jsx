...
export default function WebsiteImageUpload({onUploaded,disabled}){
 ...
 return <section className="address-section"><FormField label={bilingualLabel("Upload website image")} type="file" accept="image/jpeg,image/png,image/webp" disabled={busy||disabled} onChange={e=>{setFile(e.target.files[0]||null);id.current=crypto.randomUUID()}} hint={t("Images become public when linked to a published tour or active pop-up.")}/><Button disabled={!file||busy||disabled} busy={busy} onClick={upload}>{t("Upload and use image")}</Button>{error&&<p role="alert">{t(error)}</p>}</section>
}
