...
export function UserActions({ user, canChangeDepartment, onClose, onSaved, initialMode = 'view', self = false }) {
 useLocale();
  ...
  return <Dialog title={discard ? t('Discard changes?') : mode === 'edit' ? self ? t('Edit profile') : t('Edit user') : mode === 'review' ? t('Review department change') : mode === 'view' ? t('User details') : t('User actions')} onClose={close} busy={busy}>
    {discard ? <><p>{t("Your changes to")}{' '}{user.email}{' '}{t("have not been saved.")}</p><div className="dialog-actions"><Button autoFocus onClick={() => setDiscard(false)}>{t("Keep editing")}</Button><Button onClick={onClose}>{t("Discard changes")}</Button></div></> : <>
      ...
      {mode === 'view' ? <dl className="profile-details"><dt>{t("Name")}</dt><dd>{user.displayName}</dd><dt>{t("Department")}</dt><dd>{t(user.department || 'Not assigned')}</dd>{Object.entries({ primaryPhone: 'Primary phone', emergencyPhone: 'Emergency phone', lineId: 'Line ID', address: 'Address' }).map(([key, label]) => <div className="profile-detail-pair" key={key}><dt>{t(label)}</dt><dd>{(key==='address'?formatAddress(user):user[key]) || t("Not provided")}</dd></div>)}{mapLinks(user).pin&&<div><dt>{t("Map location")}</dt><dd><a href={mapLinks(user).pin} target="_blank" rel="noopener noreferrer">{t("Open saved pin")}</a></dd></div>}<dt>{t("Status")}</dt><dd>{t(user.status)}</dd><dt>{t("Roles")}</dt><dd>{user.roles?.map(role => t(roleNames[role.roleCode] || role.roleCode)).join(' · ')}</dd></dl> : <form ref={form} noValidate onSubmit={save} aria-busy={busy}>
        {mode === 'review' ? <><p>{t("Move this user from")}{' '}<strong>{t(user.department || 'Not assigned')}</strong>{' '}{t("to")}{' '}<strong>{t(department || 'Not assigned')}</strong>?</p><p>{t("Department assignment controls which Head can manage this profile. For Head accounts, it also controls access to their department directory.")}</p></> : <>
          <FormField label={t("Display name")} value={displayName} onChange={event => { setDisplayName(event.target.value); setFieldError('') }} error={t(fieldError)} maxLength={100} disabled={busy} autoComplete="off" />
          {canChangeDepartment ? <SelectField label={t("Department")} value={department} onChange={event => setDepartment(event.target.value)} disabled={busy} hint={t("Heads can manage profiles only in their assigned department.")}><option value="">{t("Not assigned")}</option>{departments.map(item => <option key={item} value={item}>{t(item)}</option>)}</SelectField> : <p>{t("Department:")}{' '}{t(user.department)}</p>}
          ...
          <p className="field-help">{t("Email, password, account status and roles are unchanged by this form.")}</p>
        </>}
        {error && <p className="inline-error" role="alert">{t(error)}</p>}
        <div className="dialog-actions">{mode === 'review' && <Button disabled={busy} onClick={() => setMode('edit')}>{t("Back to edit")}</Button>}<Button type="submit" className="button-primary" disabled={busy || !dirty} busy={busy}>{mode === 'review' ? t('Confirm department change') : t('Save changes')}</Button></div>
      </form>}
    </>}
  </Dialog>
}
