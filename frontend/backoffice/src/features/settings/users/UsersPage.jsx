...
export default function UsersPage({ onProfileSaved }) {
  ...
  return <>
    <section className="page-heading"><div><p className="eyebrow">YOUR TEAM, IN ONE PLACE</p><h1>Users</h1><p className="muted">A clear view of the people who access Greenview Tour.</p></div><div className="page-actions">{data?.canInvite && <Button className="button-primary" onClick={() => setInviteOpen(true)}>+ Add employee</Button>}</div></section>
    ...
    <section className="panel" aria-labelledby="directory-heading"><div className="panel-heading"><div><h2 id="directory-heading">User directory</h2><p>Manage your team’s access and profile details.</p></div><span className={`connection ${connected ? 'connected' : error ? 'disconnected' : ''}`} role="status"><span />{loading ? 'Checking connection' : connected ? 'Database connected' : 'Connection unavailable'}</span></div>
      ...
      <DataTable columns={['User', 'Department / Role', 'Email verification', 'Last sign-in', 'Actions']} busy={loading}>
        ...
        ]}><span aria-hidden="true">⋯</span></Dropdown></td></tr>) : <tr><td colSpan="5"><div className="empty-state"><span className="empty-icon"><Icon name="users" width="30" height="30" /></span><h3>{query ? 'No matching users' : 'Your team starts here'}</h3><p>{query ? 'Try another email address or clear the search.' : 'The database is connected. No user accounts have been created yet.'}</p>{query && <Button onClick={() => setSearch('')}>Clear search</Button>}</div></td></tr>}
      </DataTable>
    ...
    {data?.canInvite && <StaffInvitations open={inviteOpen} onClose={() => setInviteOpen(false)} />}
