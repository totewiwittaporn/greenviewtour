...
export default function UsersPage({ onProfileSaved }) {
 useLocale();
  ...
  return <>
    {data?.canInvite && <Tabs items={[{ id: 'users', label: 'Users' }, { id: 'invitations', label: 'Invitations' }]} value={tab} onChange={setTab} label={t("User management")} idPrefix="user-management" />}

    <section className="page-heading"><div><p className="eyebrow">{t("YOUR TEAM, IN ONE PLACE")}</p><h1>{tab === 'invitations' ? t('Invitations') : t('Users')}</h1><p className="muted">{tab === 'invitations' ? t('Invite employees and follow their account activation.') : t('A clear view of the people who access Greenview Tour.')}</p></div><div className="page-actions">{data?.canInvite && <Button className="button-primary" onClick={() => { setTab('invitations'); setInviteOpen(true) }}>{t("+ Add employee")}</Button>}</div></section>
    ...
    <SummaryCards label={t("Account summary")} items={[
    ...
    <section className="panel table-panel" aria-labelledby="directory-heading"><div className="panel-heading"><div><h2 id="directory-heading">{t("User directory")}</h2><p>{t("Manage your team’s access and profile details.")}</p></div><span className={`connection ${connected ? 'connected' : error ? 'disconnected' : ''}`} role="status"><span />{loading ? t('Checking connection') : connected ? t('Database connected') : t('Connection unavailable')}</span></div>
      ...
      <Pagination page={data?.page ?? page} pageSize={25} total={error || loading ? undefined : data?.total} busy={loading} onPageChange={setPage} label={t("Users pagination")} />
    </section>
    </TabPanel>
