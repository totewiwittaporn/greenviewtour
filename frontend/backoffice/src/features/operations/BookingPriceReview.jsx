...
export function BookingPriceReview({bookingId,onClose,onSaved}){
 ...
 return <><Dialog title={bilingualLabel("Agent negotiated price")} busy={busy} onClose={close}>{!row?<>{error?<><p role="alert">{t(error)}</p><Button onClick={()=>setRetry(n=>n+1)}>{t("Retry")}</Button></>:<p role="status">{t("Loading current booking…")}</p>}</>:<>
 ...
 {(row.priceActions||[]).includes('REQUEST')&&<form noValidate onSubmit={e=>{e.preventDefault();if(!e.currentTarget.checkValidity()){e.currentTarget.querySelector(':invalid')?.focus();setError('Complete the prices and reason.');return}save('REQUEST')}}><fieldset className="address-section" disabled={busy}><legend>{bilingualLabel("Propose price")}</legend><FormField label={bilingualLabel("Proposed adult price (THB)")} type="number" min="0" max="99999999.99" step="0.01" required value={draft.adultPrice} onChange={e=>change('adultPrice',e.target.value)}/><FormField label={bilingualLabel("Proposed child price (THB)")} type="number" min="0" max="99999999.99" step="0.01" required value={draft.childPrice} onChange={e=>change('childPrice',e.target.value)}/><TextAreaField label={bilingualLabel("Negotiation reason")} required maxLength={1000} value={draft.reason} onChange={e=>change('reason',e.target.value)}/><p>{t("Proposed total:")}{' '}{amount(proposed.total)}</p><Button type="submit" busy={busy}>{t("Request price approval")}</Button></fieldset></form>}
 {(row.priceActions||[]).some(a=>a!=='REQUEST')&&<section className="address-section"><h3>{bilingualLabel("Review saved request")}</h3><p>{t("Approval applies to the saved request displayed above, not unsaved proposal edits.")}</p><TextAreaField label={bilingualLabel("Review / withdrawal reason")} disabled={busy} maxLength={1000} value={reviewReason} onChange={e=>{setDirty(true);setReviewReason(e.target.value)}}/><div className="dialog-actions">{row.priceActions.filter(a=>a!=='REQUEST').map(action=><Button key={action} disabled={busy} onClick={()=>save(action)}>{t(labels[action])}</Button>)}</div></section>}
 ...
 </>}</Dialog>{discard&&<Dialog title={bilingualLabel("Discard price entries?")} onClose={()=>setDiscard(false)}><p>{t("These entries have not been saved.")}</p><Button onClick={()=>setDiscard(false)}>{t("Keep editing")}</Button><Button onClick={onClose}>{t("Discard changes")}</Button></Dialog>}</>
}
