...
const statusLabel={WAITING:'Waiting',PARTIAL:'Partly checked in',CHECKED_IN:'Checked in',NO_SHOW:'ไม่มาใช้บริการ',PARTIAL_NO_SHOW:'มาไม่ครบ · บางคนไม่มา'}
...
function Review({row,action,serviceDate,close,onClose,onSaved}){
 ...
 return <Dialog title={title} variant={['NO_SHOW','VIEW'].includes(action)?'table':undefined} busy={busy} onClose={onClose}>
  <form noValidate onSubmit={save}>
   ...
   {action==='VIEW'&&<><p>Arrived: {row.adults} adults / {row.children} children · No-show: {row.noShowAdults} adults / {row.noShowChildren} children</p><p>Reason: {row.reason||'—'}</p><p>Finance: {financeLabel[row.financeStatus]} · {row.financeReason||'No financial decision recorded.'}</p><DataTable label="Recorded allocation changes" columns={['Run','Original','After review','Result']} isEmpty={!row.changes.length} empty={<p>No allocation changes recorded.</p>}>{row.changes.map(c=><tr key={c.id}><td>{c.run}</td><td>{c.beforeAdults} / {c.beforeChildren}</td><td>{c.adults} / {c.children}</td><td>{c.preserved?'Recorded service preserved':c.status==='CANCELLED'?'ยกเลิก · ไม่มาใช้บริการ':'Passengers reduced'}</td></tr>)}</DataTable></>}
   {row?.demo&&<p role="status">DEMO booking · simulated payment. No real payment was received.</p>}
   {action==='CHECK_IN'&&<><p>Confirm only guests who are here now. Remaining: {row.remainingAdults} adults / {row.remainingChildren} children.</p><div className="form-grid"><FormField label="Adults arriving now" type="number" min="0" max={row.remainingAdults} step="1" value={adults} onChange={e=>setAdults(e.target.value)} disabled={busy}/><FormField label="Children arriving now" type="number" min="0" max={row.remainingChildren} step="1" value={children} onChange={e=>setChildren(e.target.value)} disabled={busy}/></div></>}
   {action==='NO_SHOW'&&<><p>Mark the remaining {row.remainingAdults} adults / {row.remainingChildren} children as ไม่มาใช้บริการ. Review each allocation below before confirming.</p><DataTable label="Allocation changes" columns={['Run','Original','After review','Result']} busy={!preview&&!error} isEmpty={Boolean(preview&&!preview.changes.length)} empty={<p>No unserved allocations need changing.</p>}>{preview?.changes.map(c=><tr key={c.id}><td>{c.run}</td><td>{c.beforeAdults} / {c.beforeChildren}</td><td>{c.adults} / {c.children}</td><td>{c.preserved?'Recorded service preserved':c.status==='CANCELLED'?'ยกเลิก · ไม่มาใช้บริการ':'Reduce passengers'}</td></tr>)}</DataTable><p>Booking prices and received funds remain unchanged. Finance must review billing, adjustment or refund conditions separately.</p></>}
   {action==='FINANCE_REVIEW'&&<><p>Record the decision and its reason. This does not issue a refund, alter a bill or move money.</p><SelectField label="Financial decision" value={decision} onChange={e=>setDecision(e.target.value)} disabled={busy}><option value="RETAIN_CHARGES">Retain agreed charges</option><option value="ADJUSTMENT_REQUIRED">Adjustment required</option><option value="REFUND_REQUIRED">Refund review required</option></SelectField></>}
   {action==='CLOSE'&&<p>Close attendance for {serviceDate}. All guests must be checked in or reviewed as no-show. Outstanding financial reviews remain open.</p>}
   {['NO_SHOW','REOPEN','FINANCE_REVIEW'].includes(action)&&<FormField label="Reason" value={reason} maxLength={1000} required onChange={e=>setReason(e.target.value)} disabled={busy}/>}
   {error&&<p role="alert">{error}</p>}<div className="dialog-actions"><Button type="button" onClick={onClose} disabled={busy}>Cancel</Button>{action!=='VIEW'&&<Button type="submit" busy={busy} disabled={busy||(action==='NO_SHOW'&&!preview)}>{title}</Button>}</div>
  </form>
 </Dialog>
}
export default function CheckInPage({actor}){
 ...
 return <><div className="page-heading"><div><h1>Customer check-in</h1><p>Verify the booking, count arrivals and confirm service attendance.</p></div></div>
  ...
  <section className="panel table-panel"><form noValidate className="filterbar" onSubmit={search}><DateField label="Service date (Thailand)" value={date} onChange={e=>setDate(e.target.value)}/><FormField label="Scan or enter booking code" value={query} maxLength={100} onChange={e=>setQuery(e.target.value)} hint="Use a keyboard barcode scanner, or enter a code / guest name."/><Button type="submit" disabled={loading}>Find booking</Button><RefreshButton disabled={loading} onClick={()=>setAttempt(n=>n+1)}/></form>
  <div className="address-section"><p>Service day: {filter.date} · {closed?'Closed':'Open'} · Financial reviews outstanding: {data?.summary.financePending??'—'}</p>{manager&&<Button disabled={loading||Boolean(error)||!data} onClick={()=>setReview({action:closed?'REOPEN':'CLOSE'})}>{closed?'Reopen service day':'Close service day'}</Button>}</div>
  {notice&&<p role="status">{notice}</p>}
  <DataTable label="Service attendance" columns={['Booking / guest','Leg','Arrived / expected','Status','Finance','Actions']} busy={loading} error={error} onRetry={()=>setAttempt(n=>n+1)} isEmpty={!data?.rows.length} empty={<p>No bookings match this service date and search. Check the date or booking code.</p>}>
   {data?.rows.map(row=><tr key={`${row.bookingId}:${row.direction}`}><td>{row.code}<div className="field-help">{row.name}</div></td><td>{row.direction==='OUTBOUND'?'Outbound':'Return'}</td><td>{row.adults+row.children} / {row.expectedAdults+row.expectedChildren}</td><td>{statusLabel[row.status]}</td><td>{financeLabel[row.financeStatus]}</td><td><Dropdown rowActions label={`Actions for ${row.code} ${row.direction}`} items={[{label:'View attendance',icon:'view',onSelect:()=>setReview({row,action:'VIEW'})},{label:'Confirm arrivals',icon:'check',disabled:closed||!row.remainingAdults&&!row.remainingChildren,onSelect:()=>setReview({row,action:'CHECK_IN'})},...(manager?[{label:'Review no-show',icon:'edit',disabled:closed||!row.remainingAdults&&!row.remainingChildren,onSelect:()=>setReview({row,action:'NO_SHOW'})}]:[]),...(row.financeStatus!=='NONE'?[{label:'Review financial impact',icon:'view',onSelect:()=>setReview({row,action:'FINANCE_REVIEW'})}]:[])]}/></td></tr>)}
