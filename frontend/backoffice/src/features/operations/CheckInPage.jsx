...
function Review({row,action,serviceDate,close,onClose,onSaved}){
 ...
 return <Dialog title={title} variant={['NO_SHOW','VIEW'].includes(action)?'table':undefined} busy={busy} onClose={onClose}>
  <form noValidate onSubmit={save}>
   ...
   {action==='VIEW'&&<><p>{t("Arrived:")}{' '}{row.adults}{' '}{t("adults /")}{' '}{row.children}{' '}{t("children · No-show:")}{' '}{row.noShowAdults}{' '}{t("adults /")}{' '}{row.noShowChildren}{' '}{t("children")}</p><p>{t("Reason:")}{' '}{row.reason||'—'}</p><p>{t("Finance:")}{' '}{t(financeLabel[row.financeStatus])} · {row.financeReason||t("No financial decision recorded.")}</p><DataTable label={t("Recorded allocation changes")} columns={['Run','Original','After review','Result']} isEmpty={!row.changes.length} empty={<p>{t("No allocation changes recorded.")}</p>}>{row.changes.map(c=><tr key={c.id}><td>{c.run}</td><td>{c.beforeAdults} / {c.beforeChildren}</td><td>{c.adults} / {c.children}</td><td>{c.preserved?t('Recorded service preserved'):c.status==='CANCELLED'?t("Cancelled · no-show"):t('Passengers reduced')}</td></tr>)}</DataTable></>}
   {row?.demo&&<p role="status">{t("DEMO booking · simulated payment. No real payment was received.")}</p>}
   {action==='CHECK_IN'&&<><p>{t("Confirm only guests who are here now. Remaining:")}{' '}{row.remainingAdults}{' '}{t("adults /")}{' '}{row.remainingChildren}{' '}{t("children.")}</p><div className="form-grid"><FormField label={t("Adults arriving now")} type="number" min="0" max={row.remainingAdults} step="1" value={adults} onChange={e=>setAdults(e.target.value)} disabled={busy}/><FormField label={t("Children arriving now")} type="number" min="0" max={row.remainingChildren} step="1" value={children} onChange={e=>setChildren(e.target.value)} disabled={busy}/></div></>}
   {action==='NO_SHOW'&&<><p>{t("Mark the remaining")}{' '}{row.remainingAdults}{' '}{t("adults /")}{' '}{row.remainingChildren}{' '}{t("children as no-show. Review each allocation below before confirming.")}</p><DataTable label={t("Allocation changes")} columns={['Run','Original','After review','Result']} busy={!preview&&!error} isEmpty={Boolean(preview&&!preview.changes.length)} empty={<p>{t("No unserved allocations need changing.")}</p>}>{preview?.changes.map(c=><tr key={c.id}><td>{c.run}</td><td>{c.beforeAdults} / {c.beforeChildren}</td><td>{c.adults} / {c.children}</td><td>{c.preserved?t('Recorded service preserved'):c.status==='CANCELLED'?t("Cancelled · no-show"):t('Reduce passengers')}</td></tr>)}</DataTable><p>{t("Booking prices and received funds remain unchanged. Finance must review billing, adjustment or refund conditions separately.")}</p></>}
   {action==='FINANCE_REVIEW'&&<><p>{t("Record the decision and its reason. This does not issue a refund, alter a bill or move money.")}</p><SelectField label={t("Financial decision")} value={decision} onChange={e=>setDecision(e.target.value)} disabled={busy}><option value="RETAIN_CHARGES">{t("Retain agreed charges")}</option><option value="ADJUSTMENT_REQUIRED">{t("Adjustment required")}</option><option value="REFUND_REQUIRED">{t("Refund review required")}</option></SelectField></>}
   ...
   {['NO_SHOW','REOPEN','FINANCE_REVIEW'].includes(action)&&<FormField label={t("Reason")} value={reason} maxLength={1000} required onChange={e=>setReason(e.target.value)} disabled={busy}/>}
...
export default function CheckInPage({actor}){
 ...
 return <><div className="page-heading"><div><h1>{t("Customer check-in")}</h1><p>{t("Verify the booking, count arrivals and confirm service attendance.")}</p></div></div>
  ...
  <section className="panel table-panel"><form noValidate className="filterbar" onSubmit={search}><DateField label={t("Service date (Thailand)")} value={date} onChange={e=>setDate(e.target.value)}/><FormField label={t("Scan or enter booking code")} value={query} maxLength={100} onChange={e=>setQuery(e.target.value)} hint={t("Use a keyboard barcode scanner, or enter a code / guest name.")}/><Button type="submit" disabled={loading}>{t("Find booking")}</Button><RefreshButton disabled={loading} onClick={()=>setAttempt(n=>n+1)}/></form>
  ...
  <DataTable label={t("Service attendance")} columns={['Booking / guest','Leg','Arrived / expected','Status','Finance','Actions']} busy={loading} error={t(error)} onRetry={()=>setAttempt(n=>n+1)} isEmpty={!data?.rows.length} empty={<p>{t("No bookings match this service date and search. Check the date or booking code.")}</p>}>
   {data?.rows.map(row=><tr key={`${row.bookingId}:${row.direction}`}><td>{row.code}<div className="field-help">{row.name}</div></td><td>{row.direction==='OUTBOUND'?t('Outbound'):t('Return')}</td><td>{row.adults+row.children} / {row.expectedAdults+row.expectedChildren}</td><td>{t(statusLabel[row.status])}</td><td>{t(financeLabel[row.financeStatus])}</td><td><Dropdown rowActions label={t("Actions for {value0} {value1}", {value0: row.code, value1: row.direction})} items={[{label:'View attendance',icon:'view',onSelect:()=>setReview({row,action:'VIEW'})},{label:'Confirm arrivals',icon:'check',disabled:closed||!row.remainingAdults&&!row.remainingChildren,onSelect:()=>setReview({row,action:'CHECK_IN'})},...(manager?[{label:'Review no-show',icon:'edit',disabled:closed||!row.remainingAdults&&!row.remainingChildren,onSelect:()=>setReview({row,action:'NO_SHOW'})}]:[]),...(row.financeStatus!=='NONE'?[{label:'Review financial impact',icon:'view',onSelect:()=>setReview({row,action:'FINANCE_REVIEW'})}]:[])]}/></td></tr>)}
