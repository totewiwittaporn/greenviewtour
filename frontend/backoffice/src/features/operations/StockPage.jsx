...
function StockCommand({ action, row, onClose, onSaved }) {
 ...
 {action === 'CONDITION' && <SelectField label="New condition" value={values.condition} onChange={event => change('condition', event.target.value)} disabled={busy}><option value="READY">Ready</option><option value="DAMAGED">Damaged</option></SelectField>}
 {action === 'SETTLE' && <><SelectField label="Disposition" value={values.disposition} onChange={event => change('disposition', event.target.value)} disabled={busy}><option value="RETURN_READY">Return ready (intact items only)</option><option value="RETURN_DAMAGED">Return damaged</option>{resource?.kind !== 'EQUIPMENT' && <option value="CONSUMED">Consumed</option>}<option value="WASTED">Lost / wasted</option></SelectField>{values.disposition.startsWith('RETURN') && <Reference entity="stores" label="Return location" value={values.locationId} onChange={value => change('locationId', value)} disabled={busy}/>}</>}
...
export default function StockPage({ entity = 'stock' }) {
 ...
 {notice && <p role="status">{notice}</p>}<section className="panel table-panel"><div className="filterbar"><SearchField label={`Search ${title.toLowerCase()}`} value={query} onChange={value => { setQuery(value); setPage(1) }} onCompositionChange={setComposing}/><RefreshButton onClick={() => setRefresh(value => value + 1)} disabled={state.loading}/></div>
 <DataTable columns={columns} label={title} busy={state.loading} error={state.error} onRetry={() => setRefresh(value => value + 1)} isEmpty={!state.rows.length} empty={<p>{query.trim()?'No matching records. Clear the search or try an item, lot, location or note.':entity==='movements'?'No stock movements recorded yet.':'No stock balances recorded yet.'}</p>}>{state.rows.map(row => <tr key={row.id}><td>{entity === 'movements' ? new Date(row.createdAt).toLocaleString('en-GB', { timeZone: 'Asia/Bangkok' }) : <>{row.lot?.resource?.name}<div>{row.lot?.label}</div></>}</td>
 {entity === 'stock' ? <><td>{row.location?.name || row.location?.label}</td><td>{readable(row.condition)}</td><td>{quantity(row.quantity)} {row.lot?.resource?.baseUnit}</td><td>{row.lot?.expiresOn ? String(row.lot.expiresOn).slice(0,10) : '—'}</td><td><Dropdown rowActions label={`Actions for ${row.lot?.resource?.name}`} items={[{ label: 'Transfer', icon: 'arrow', onSelect: () => setSelection({ action: 'TRANSFER', row }) }, { label: 'Issue', icon: 'briefcase', disabled: row.condition !== 'READY', onSelect: () => setSelection({ action: 'ISSUE', row }) }, { label: 'Count stock', icon: 'check', onSelect: () => navigate('/company/stock-counts') }, { label: 'Change condition', icon: 'edit', onSelect: () => setSelection({ action: 'CONDITION', row }) }]}/></td></> : entity === 'issues' ? <><td>{row.destination?.name || row.destination?.label}</td><td>{row.custodian}</td><td>{quantity(row.quantity)} {row.lot?.resource?.baseUnit}</td><td>{quantity(Number(row.quantity) - Number(row.settledQty))}</td><td><Dropdown rowActions label={`Actions for ${row.lot?.resource?.name}`} items={[{ label: 'Return / settle', icon: 'check', disabled: Number(row.quantity) <= Number(row.settledQty), onSelect: () => setSelection({ action: 'SETTLE', row }) }]}/></td></> : <><td>{row.details?.resourceName || '—'}<div>{row.details?.lotLabel}</div></td><td>{readable(row.kind)}<div>{readable(row.details?.disposition)}</div></td><td>{quantity(row.quantity)} {row.details?.baseUnit}<div>{quantity(row.enteredQuantity)} {readable(row.enteredUnit)} · factor {quantity(row.factor)}</div></td><td>{row.details?.sourceName || '—'} → {row.details?.destinationName || '—'}<div>{row.details?.note}</div></td></>}
