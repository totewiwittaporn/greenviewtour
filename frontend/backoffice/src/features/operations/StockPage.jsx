...
export default function StockPage({ entity = 'stock' }) {
 ...
 {entity === 'stock' ? <><td>{row.location?.name || row.location?.label}</td><td>{readable(row.condition)}</td><td>{quantity(row.quantity)} {row.lot?.resource?.baseUnit}</td><td>{row.lot?.expiresOn ? String(row.lot.expiresOn).slice(0,10) : '—'}</td><td><Dropdown label={`Actions for ${row.lot?.resource?.name}`} items={[{ label: 'Transfer', icon: 'arrow', onSelect: () => setSelection({ action: 'TRANSFER', row }) }, { label: 'Issue', icon: 'briefcase', disabled: row.condition !== 'READY', onSelect: () => setSelection({ action: 'ISSUE', row }) }, { label: 'Count stock', icon: 'check', onSelect: () => navigate('/company/stock-counts') }, { label: 'Change condition', icon: 'edit', onSelect: () => setSelection({ action: 'CONDITION', row }) }]}><span aria-hidden="true">⋯</span></Dropdown></td></> : entity === 'issues' ? <><td>{row.destination?.name || row.destination?.label}</td><td>{row.custodian}</td><td>{quantity(row.quantity)} {row.lot?.resource?.baseUnit}</td><td>{quantity(Number(row.quantity) - Number(row.settledQty))}</td><td><Dropdown label={`Actions for ${row.lot?.resource?.name}`} items={[{ label: 'Return / settle', icon: 'check', disabled: Number(row.quantity) <= Number(row.settledQty), onSelect: () => setSelection({ action: 'SETTLE', row }) }]}><span aria-hidden="true">⋯</span></Dropdown></td></> : <><td>{row.details?.resourceName || '—'}<div>{row.details?.lotLabel}</div></td><td>{readable(row.kind)}<div>{readable(row.details?.disposition)}</div></td><td>{quantity(row.quantity)} {row.details?.baseUnit}<div>{quantity(row.enteredQuantity)} {readable(row.enteredUnit)} · factor {quantity(row.factor)}</div></td><td>{row.details?.sourceName || '—'} → {row.details?.destinationName || '—'}<div>{row.details?.note}</div></td></>}
