...
function StockCommand({ action, row, onClose, onSaved }) {
 ...
 return <><Dialog title={t("{value0} stock", {value0: readable(action)})} busy={busy} onClose={close}><form noValidate onSubmit={submit}>
 ...
 {action === 'RECEIVE' && <><Reference entity="resources" label={t("Resource")} value={values.resourceId} onChange={value => change('resourceId', value)} onRows={collect} disabled={busy}/><Reference entity="stores" label={t("Receiving location")} value={values.locationId} onChange={value => change('locationId', value)} disabled={busy}/>{field('lotLabel', 'Lot / delivery reference', { required: true })}{field('receivedOn', 'Received on', { placeholder: 'YYYY-MM-DD', required: true, hint: 'Bangkok date · YYYY-MM-DD' })}{field('expiresOn', 'Expires on', { placeholder: 'YYYY-MM-DD', hint: 'Optional · YYYY-MM-DD' })}{field('unitCost', 'Unit cost (THB)', { inputMode: 'decimal', hint: 'Optional · cost per selected purchase unit' })}</>}
 {['TRANSFER', 'ISSUE'].includes(action) && <Reference entity="stores" label={t("Destination")} value={values.destinationId} onChange={value => change('destinationId', value)} disabled={busy}/>}
 {action === 'ISSUE' && <>{field('custodian', 'Responsible person', { required: true })}<Reference entity="bookings" label={t("Booking (optional)")} value={bookingId} onChange={value => { setBookingId(value); change('bookingLineId', '') }} onRows={collectBookings} disabled={busy}/>{bookingId && <SelectField label={t("Booking resource line")} value={values.bookingLineId} onChange={event => change('bookingLineId', event.target.value)} disabled={busy}><option value="">{t("Select a matching line")}</option>{(bookings[bookingId]?.lines || []).filter(line => bookings[bookingId]?.status === 'CONFIRMED' && line.selected && line.resourceId === resource?.id && line.sourceId === row?.locationId && Number(line.quantity) > Number(line.issuedQty)).map(line => <option key={line.id} value={line.id}>{line.resource?.name} · {quantity(Number(line.quantity) - Number(line.issuedQty))}{' '}{t("remaining")}</option>)}</SelectField>}</>}
 {action === 'CONDITION' && <SelectField label={t("New condition")} value={values.condition} onChange={event => change('condition', event.target.value)} disabled={busy}><option value="READY">{t("Ready")}</option><option value="DAMAGED">{t("Damaged")}</option></SelectField>}
 {action === 'SETTLE' && <><SelectField label={t("Disposition")} value={values.disposition} onChange={event => change('disposition', event.target.value)} disabled={busy}><option value="RETURN_READY">{t("Return ready (intact items only)")}</option><option value="RETURN_DAMAGED">{t("Return damaged")}</option>{resource?.kind !== 'EQUIPMENT' && <option value="CONSUMED">{t("Consumed")}</option>}<option value="WASTED">{t("Lost / wasted")}</option></SelectField>{values.disposition.startsWith('RETURN') && <Reference entity="stores" label={t("Return location")} value={values.locationId} onChange={value => change('locationId', value)} disabled={busy}/>}</>}
 {action === 'COUNT' ? field('countedQuantity', `Counted quantity (${resource?.baseUnit || 'base units'})`, { type: 'number', min: 0, step: 1, required: true }) : <>{field('quantity', 'Quantity', { type: 'number', min: 1, step: 1, required: true })}<SelectField label={t("Unit")} value={values.unit} onChange={event => change('unit', event.target.value)} disabled={busy}><option value="BASE">{resource?.baseUnit || 'Base unit'}</option>{resource?.baseUnit === 'BOTTLE' && Number(resource?.packSize) > 0 && <option value="PACK">{t("Pack ·")}{' '}{resource.packSize}{' '}{t("base units")}</option>}{resource?.baseUnit === 'BOTTLE' && Number(resource?.caseSize) > 0 && <option value="CASE">{t("Case ·")}{' '}{resource.caseSize}{' '}{t("base units")}</option>}</SelectField></>}
 ...
 </form></Dialog>{discard && <Dialog title={t("Discard unsaved changes?")} onClose={() => setDiscard(false)}><p>{t("Your stock transaction has not been saved.")}</p><div className="dialog-actions"><Button onClick={() => setDiscard(false)}>{t("Keep editing")}</Button><Button onClick={onClose}>{t("Discard changes")}</Button></div></Dialog>}</>
}
export default function StockPage({ entity = 'stock' }) {
 ...
 return <><header className="page-heading"><div><h1 tabIndex={-1}>{t(title)}</h1><p>{entity === 'movements' ? t('Permanent transaction history for stock reconciliation.') : t('Track equipment and consumables by location and lot.')}</p></div>{entity === 'stock' && <Button onClick={() => setSelection({ action: 'RECEIVE' })}>{t("Receive stock")}</Button>}</header>
 <SummaryCards label={t("Stock summary · before filters")} items={[{ label: entity === 'stock' ? 'Stock balances' : 'Total records', value: state.summary?.total, detail: 'All records · before filters' }, { label: entity === 'stock' ? 'Ready balances' : entity === 'issues' ? 'Outstanding issues' : 'Recorded movements', value: state.summary?.active }, { label: entity === 'stock' ? 'Legacy cleaning balances' : 'History records', value: state.summary?.featured }, { label: entity === 'stock' ? 'Not ready' : entity === 'issues' ? 'Fully settled' : 'Removed records', value: state.summary?.inactive }]}/>
 {notice && <p role="status">{t(notice)}</p>}<section className="panel table-panel"><div className="filterbar"><SearchField label={t("Search {value0}", {value0: title.toLowerCase()})} value={query} onChange={value => { setQuery(value); setPage(1) }} onCompositionChange={setComposing}/><RefreshButton onClick={() => setRefresh(value => value + 1)} disabled={state.loading}/></div>
