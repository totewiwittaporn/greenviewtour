...
import { useNavigation, useUnsavedChanges } from '../../core/navigation/Navigation.jsx'
...
const quantity = value => formatNumber(Number(value || 0), { maximumFractionDigits: 3 })
const errorMessage = error => `Unable to save or load records: ${error.detail || t(readable(error.message))}. Your entries are preserved. For a changed balance, close this form and refresh before trying again.`
...
function StockCommand({ action, row, onClose, onSaved }) {
 ...
 return <><Dialog title={bilingualLabel("{value0} stock", {value0: readable(action)})} busy={busy} onClose={close}><form noValidate onSubmit={submit}>
 ...
 {action === 'RECEIVE' && <><Reference entity="resources" label={bilingualLabel("Resource")} value={values.resourceId} onChange={value => change('resourceId', value)} onRows={collect} disabled={busy}/><Reference entity="stores" label={bilingualLabel("Receiving location")} value={values.locationId} onChange={value => change('locationId', value)} disabled={busy}/>{field('lotLabel', 'Lot / delivery reference', { required: true })}{field('receivedOn', 'Received on', { placeholder: 'YYYY-MM-DD', required: true, hint: 'Bangkok date · YYYY-MM-DD' })}{field('expiresOn', 'Expires on', { placeholder: 'YYYY-MM-DD', hint: 'Optional · YYYY-MM-DD' })}{field('unitCost', 'Unit cost (THB)', { inputMode: 'decimal', hint: 'Optional · cost per selected purchase unit' })}</>}
 {['TRANSFER', 'ISSUE'].includes(action) && <Reference entity="stores" label={bilingualLabel("Destination")} value={values.destinationId} onChange={value => change('destinationId', value)} disabled={busy}/>}
 {action === 'ISSUE' && <>{field('custodian', 'Responsible person', { required: true })}<Reference entity="bookings" label={bilingualLabel("Booking (optional)")} value={bookingId} onChange={value => { setBookingId(value); change('bookingLineId', '') }} onRows={collectBookings} disabled={busy}/>{bookingId && <SelectField label={bilingualLabel("Booking resource line")} value={values.bookingLineId} onChange={event => change('bookingLineId', event.target.value)} disabled={busy}><option value="">{t("Select a matching line")}</option>{(bookings[bookingId]?.lines || []).filter(line => bookings[bookingId]?.status === 'CONFIRMED' && line.selected && line.resourceId === resource?.id && line.sourceId === row?.locationId && Number(line.quantity) > Number(line.issuedQty)).map(line => <option key={line.id} value={line.id}>{line.resource?.name} · {quantity(Number(line.quantity) - Number(line.issuedQty))}{' '}{t("remaining")}</option>)}</SelectField>}</>}
 {action === 'CONDITION' && <SelectField label={bilingualLabel("New condition")} value={values.condition} onChange={event => change('condition', event.target.value)} disabled={busy}><option value="READY">{t("Ready")}</option><option value="DAMAGED">{t("Damaged")}</option></SelectField>}
 {action === 'SETTLE' && <><SelectField label={bilingualLabel("Disposition")} value={values.disposition} onChange={event => change('disposition', event.target.value)} disabled={busy}><option value="RETURN_READY">{t("Return ready (intact items only)")}</option><option value="RETURN_DAMAGED">{t("Return damaged")}</option>{resource?.kind !== 'EQUIPMENT' && <option value="CONSUMED">{t("Consumed")}</option>}<option value="WASTED">{t("Lost / wasted")}</option></SelectField>{values.disposition.startsWith('RETURN') && <Reference entity="stores" label={bilingualLabel("Return location")} value={values.locationId} onChange={value => change('locationId', value)} disabled={busy}/>}</>}
 {action === 'COUNT' ? field('countedQuantity', `Counted quantity (${resource?.baseUnit || 'base units'})`, { type: 'number', min: 0, step: 1, required: true }) : <>{field('quantity', 'Quantity', { type: 'number', min: 1, step: 1, required: true })}<SelectField label={bilingualLabel("Unit")} value={values.unit} onChange={event => change('unit', event.target.value)} disabled={busy}><option value="BASE">{resource?.baseUnit || 'Base unit'}</option>{resource?.baseUnit === 'BOTTLE' && Number(resource?.packSize) > 0 && <option value="PACK">{t("Pack ·")}{' '}{resource.packSize}{' '}{t("base units")}</option>}{resource?.baseUnit === 'BOTTLE' && Number(resource?.caseSize) > 0 && <option value="CASE">{t("Case ·")}{' '}{resource.caseSize}{' '}{t("base units")}</option>}</SelectField></>}
 ...
 {error && <p role="alert" className="form-error">{t(error)}</p>}<div className="dialog-actions"><Button type="submit" busy={busy} disabled={busy || (action === 'ISSUE' && bookingId && !values.bookingLineId) || (action === 'RECEIVE' && (!values.resourceId || !values.locationId)) || (['TRANSFER','ISSUE'].includes(action) && !values.destinationId) || (action === 'SETTLE' && values.disposition.startsWith('RETURN') && !values.locationId)}>{t("Save stock transaction")}</Button></div>
 </form></Dialog>{discard && <Dialog title={bilingualLabel("Discard unsaved changes?")} onClose={() => setDiscard(false)}><p>{t("Your stock transaction has not been saved.")}</p><div className="dialog-actions"><Button onClick={() => setDiscard(false)}>{t("Keep editing")}</Button><Button onClick={onClose}>{t("Discard changes")}</Button></div></Dialog>}</>
}
export default function StockPage({ entity = 'stock' }) {
 ...
 return <><header className="page-heading"><div><h1 tabIndex={-1}>{bilingualLabel(title)}</h1><p>{entity === 'movements' ? t('Permanent transaction history for stock reconciliation.') : t('Track equipment and consumables by location and lot.')}</p></div>{entity === 'stock' && <Button onClick={() => setSelection({ action: 'RECEIVE' })}>{t("Receive stock")}</Button>}</header>
 <SummaryCards label={bilingualLabel("Stock summary · before filters")} items={[{ label: entity === 'stock' ? 'Stock balances' : 'Total records', value: state.summary?.total, detail: 'All records · before filters' }, { label: entity === 'stock' ? 'Ready balances' : entity === 'issues' ? 'Outstanding issues' : 'Recorded movements', value: state.summary?.active }, { label: entity === 'stock' ? 'Legacy cleaning balances' : 'History records', value: state.summary?.featured }, { label: entity === 'stock' ? 'Not ready' : entity === 'issues' ? 'Fully settled' : 'Removed records', value: state.summary?.inactive }]}/>
 {notice && <p role="status">{t(notice)}</p>}<section className="panel table-panel"><div className="filterbar"><SearchField label={bilingualLabel("Search {value0}", {value0: title.toLowerCase()})} value={query} onChange={value => { setQuery(value); setPage(1) }} onCompositionChange={setComposing}/><RefreshButton onClick={() => setRefresh(value => value + 1)} disabled={state.loading}/></div>
 <DataTable columns={columns} label={title} busy={state.loading} error={state.error} onRetry={() => setRefresh(value => value + 1)} isEmpty={!state.rows.length} empty={<p>{query.trim()?t('No matching records. Clear the search or try an item, lot, location or note.'):entity==='movements'?t('No stock movements recorded yet.'):t('No stock balances recorded yet.')}</p>}>{state.rows.map(row => <tr key={row.id}><td>{entity === 'movements' ? displayDate(new Date(row.createdAt), {dateStyle:'short', timeStyle:'medium', ...{ timeZone: 'Asia/Bangkok' }}) : <>{row.lot?.resource?.name}<div>{row.lot?.label}</div></>}</td>
 {entity === 'stock' ? <><td>{row.location?.name || row.location?.label}</td><td>{t(readable(row.condition))}</td><td>{quantity(row.quantity)} {row.lot?.resource?.baseUnit}</td><td>{row.lot?.expiresOn ? String(row.lot.expiresOn).slice(0,10) : '—'}</td><td><Dropdown rowActions label={bilingualLabel("Actions for {value0}", {value0: row.lot?.resource?.name})} items={[{ label: 'Transfer', icon: 'arrow', onSelect: () => setSelection({ action: 'TRANSFER', row }) }, { label: 'Issue', icon: 'briefcase', disabled: row.condition !== 'READY', onSelect: () => setSelection({ action: 'ISSUE', row }) }, { label: 'Count stock', icon: 'check', onSelect: () => navigate('/company/stock-counts') }, { label: 'Change condition', icon: 'edit', onSelect: () => setSelection({ action: 'CONDITION', row }) }]}/></td></> : entity === 'issues' ? <><td>{row.destination?.name || row.destination?.label}</td><td>{row.custodian}</td><td>{quantity(row.quantity)} {row.lot?.resource?.baseUnit}</td><td>{quantity(Number(row.quantity) - Number(row.settledQty))}</td><td><Dropdown rowActions label={bilingualLabel("Actions for {value0}", {value0: row.lot?.resource?.name})} items={[{ label: 'Return / settle', icon: 'check', disabled: Number(row.quantity) <= Number(row.settledQty), onSelect: () => setSelection({ action: 'SETTLE', row }) }]}/></td></> : <><td>{row.details?.resourceName || '—'}<div>{row.details?.lotLabel}</div></td><td>{t(readable(row.kind))}<div>{t(readable(row.details?.disposition))}</div></td><td>{quantity(row.quantity)} {row.details?.baseUnit}<div>{quantity(row.enteredQuantity)} {t(readable(row.enteredUnit))}{' '}{t("· factor")}{' '}{quantity(row.factor)}</div></td><td>{row.details?.sourceName || '—'} → {row.details?.destinationName || '—'}<div>{row.details?.note}</div></td></>}
