...
export function DataTable({ columns, children, busy, label = 'Data table', error, onRetry, isEmpty = false, empty, loadingLabel = 'Loading records…', layout = 'dataset' }) {
  ...
  return <div className="table-scroll" data-layout={layout} data-columns={columns.length} tabIndex="0" role="region" aria-label={label} aria-busy={busy}><table><colgroup>{columns.map((col,i)=><col key={typeof col==='string'?col:col.label} style={{width:`${weights[i]/totalWeight*100}%`}}/>)}</colgroup><caption className="sr-only">{label}</caption><thead><tr>{columns.map(col => <th scope="col" key={typeof col==='string'?col:col.label}><span className="table-cell-copy">{typeof col==='string'?col:col.label}</span></th>)}</tr></thead><tbody>{state ? <tr><td colSpan={columns.length}>{state}</td></tr> : compactRows(children)}</tbody></table></div>
}
