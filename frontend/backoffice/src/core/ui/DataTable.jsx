...
export function DataTable({ columns, children, busy, label = 'Data table', error, onRetry, isEmpty = false, empty, loadingLabel = 'Loading records…' }) {
  ...
  return <div className="table-scroll" tabIndex="0" role="region" aria-label={label} aria-busy={busy}><table><caption className="sr-only">{label}</caption><thead><tr>{columns.map(col => <th scope="col" key={col}>{col}</th>)}</tr></thead><tbody>{state ? <tr><td colSpan={columns.length}>{state}</td></tr> : children}</tbody></table></div>
}
