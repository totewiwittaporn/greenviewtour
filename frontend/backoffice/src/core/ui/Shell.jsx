  ...
  return <div className="workspace"><a className="skip-link" href="#main">Skip to content</a>
    <aside className={`sidebar ${open ? 'is-open' : ''}`} id="workspace-navigation">
      ...
      <div className="nav-label">WORKSPACE <span>LOCAL</span></div><nav aria-label="Main navigation">
        {operationGroups.filter(group => !group.section && group.id==='booking' && canUseOperation(user, group)).map(group => <a key={group.id} href={navigation.hrefFor(`/operations/${group.entities[0]}`)} aria-current={active('operations',group)?'page':undefined} className={`nav-item ${active('operations',group)?'selected':''}`}><Icon name={group.icon}/>{group.label}</a>)}
        {operationGroups.some(group=>group.section && canUseOperation(user,group))&&<div className="nav-section-label">Operations</div>}
        <div className="nav-submenu">{operationGroups.filter(group=>group.section && canUseOperation(user,group)).map(group=><a key={group.id} href={navigation.hrefFor(`/operations/${group.entities[0]}`)} aria-current={active('operations',group)?'page':undefined} className={`nav-item ${active('operations',group)?'selected':''}`}><Icon name={group.icon}/>{group.label}</a>)}</div>
        {operationGroups.filter(group=>!group.section && group.id!=='booking' && canUseOperation(user,group) && (group.id!=='daily-summary'||user?.management?.company)).map(group=><a key={group.id} href={navigation.hrefFor(`/operations/${group.entities[0]}`)} aria-current={active('operations',group)?'page':undefined} className={`nav-item ${active('operations',group)?'selected':''}`}><Icon name={group.icon}/>{group.label}</a>)}
