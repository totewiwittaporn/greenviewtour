...
const planned = [['Dashboard', 'grid'], ['Sales', 'briefcase'], ['Tour Operations', 'globe'], ['Customers', 'users'], ['Employees', 'users'], ['Fleet', 'briefcase'], ['Finance', 'briefcase'], ['Reports', 'grid']]
  ...
  return <div className="workspace"><a className="skip-link" href="#main">Skip to content</a>
    <aside className={`sidebar ${open ? 'is-open' : ''}`} id="workspace-navigation">
      ...
      <div className="nav-label">WORKSPACE <span>LOCAL</span></div><nav aria-label="Main navigation">
        {user?.management?.company && operationGroups.map(group => <a key={group.id} href={navigation.hrefFor(`/operations/${group.entities[0]}`)} aria-current={group.entities.includes(entity) ? 'page' : undefined} className={`nav-item ${group.entities.includes(entity) ? 'selected' : ''}`}><Icon name={group.icon} />{group.label}</a>)}
