...
export function AddressPicker({values,onApply,onClose}) {
 ...
 function apply(event) {
  ...
  onApply({...draft,province:province?.th||'',district:district?.th||'',subdistrict:subdistrict?.th||''})
 }
 ...
 <p className="legacy-address" aria-live="polite">{formatAddress(draft)||'No address selected.'}</p><div className="dialog-actions"><Button disabled={!data} onClick={()=>{setDraft(snapshot({}));setErrors({})}}>Clear address</Button><Button type="submit" className="button-primary" disabled={!data}>Use this address</Button></div><p className="field-help">This updates the form. Save changes on the page to finish.</p>
