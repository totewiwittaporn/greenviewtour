...
export function AddressPicker({values,onApply,onClose}) {
 ...
 <fieldset className="address-section"><legend>{bilingualLabel("House details (optional)")}</legend><p className="field-help">{t(complete?'Leave unneeded details empty':'Select a subdistrict to enter house details.')}</p>{addressFields.filter(f=>['houseNumber','moo','villageName'].includes(f.key)).map(field=><FormField key={field.key} label={field.label} placeholder={field.placeholder} value={draft[field.key]} error={errors[field.key]} maxLength={field.max} disabled={!complete} onChange={e=>change(field.key,e.target.value)}/>)}</fieldset>
 </>}
