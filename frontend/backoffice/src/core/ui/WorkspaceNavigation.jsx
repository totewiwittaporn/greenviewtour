...
export function WorkspaceNavigation({user,canReadUsers,pageTitle}) {
 ...
 const link=(kind,group)=><a key={`${kind}-${group.id}`} href={navigation.hrefFor(`/${kind}/${group.entities[0]}`)} aria-current={active(kind,group)?'page':undefined} className={`nav-item ${active(kind,group)?'selected':''}`}><Icon name={group.icon||'briefcase'}/>{t(group.label)}</a>
 return <nav aria-label={t("Main navigation")}><a className={`nav-item ${route?.kind==='dashboard'?'selected':''}`} aria-current={route?.kind==='dashboard'?'page':undefined} href="/dashboard"><Icon name="grid"/>{t("Dashboard")}</a>{user?.management?.company&&<a className={`nav-item ${route?.kind==='customers'?'selected':''}`} href="/customers"><Icon name="users"/>{t("Customers")}</a>}{sections.map(section=>{
  ...
  return <div key={section.label}><div className="nav-section-label">{t(section.label)}</div>{operations.map(group=>link('operations',group))}{settings.map(group=>link('settings',group))}
   {section.users&&canReadUsers&&<a href="/settings/users" aria-current={pageTitle==='Users'?'page':undefined} className={`nav-item ${pageTitle==='Users'?'selected':''}`}><Icon name="users"/>{t("Users")}</a>}
   {company.map(([key,r])=><a key={key} href={navigation.hrefFor('/company/'+key)} aria-current={route?.kind==='company'&&route.entity===key?'page':undefined} className={`nav-item ${route?.kind==='company'&&route.entity===key?'selected':''}`}><Icon name="briefcase"/>{t(r.title)}</a>)}
  </div>
 })}</nav>
}
