...
export function FormField({ label, error, hint, type = 'text', ...props }) {
  ...
  return <div className="form-field"><label htmlFor={id}>{label}</label><div className="field-control">
    <input id={id} type={secret && visible ? 'text' : type} aria-invalid={Boolean(error)} aria-describedby={`${id}-help`} {...props} />
