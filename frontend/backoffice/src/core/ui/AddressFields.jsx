...
export function AddressFields({values,onChange,errors={},disabled=false,legacyAddress,quick=false}){
 ...
 return <><fieldset className="address-section"><legend>{bilingualLabel("Address")}</legend><div className="address-summary"><p className="field-help">{t(quick?'Enter your address below or use Quick address to fill these fields.':'Enter your address below.')}</p>{quick&&<Button disabled={disabled} onClick={()=>setOpen(true)}>{t("Quick address")}</Button>}</div><div className="address-grid"><GeographyFields values={values} areas={areas} onChange={onChange} errors={errors} disabled={disabled}/>{addressFields.filter(f=>f.section==='Address'&&!['province','district','subdistrict'].includes(f.key)).map(f=><FormField key={f.key} label={f.label} value={f.key==='postalCode'?postal:values[f.key]||''} onChange={e=>update(f.key,e.target.value)} error={errors[f.key]} disabled={disabled} readOnly={f.key==='postalCode'} hint={f.key==='postalCode'?(postal?'Filled automatically.':'Enter province, district and subdistrict; some areas also need a Moo number.'):undefined} placeholder={f.placeholder} maxLength={f.max}/>)}</div>{failed&&<div role="alert"><p>{t("Unable to load postal codes.")}</p><Button disabled={disabled} onClick={()=>{setFailed(false);setAttempt(n=>n+1)}}>{t("Retry postal codes")}</Button></div>}{legacyAddress&&<details><summary>{t("Previous address")}</summary><p className="legacy-address">{legacyAddress}</p></details>}</fieldset><MapLocationField values={values} onChange={onChange} error={errors.mapUrl} disabled={disabled}/>{open&&<AddressPicker values={values} onClose={()=>setOpen(false)} onApply={draft=>{for(const[key,value]of Object.entries(draft))onChange(key,value);setOpen(false)}}/>}</>
}
