import { useLocale, label as headingLabel, t as msg, formatNumber, formatMoney } from "../core/locale.js";import LeaveGuard from '../core/LeaveGuard.jsx';
...
export default function TourRequest({ tour, customer }) {useLocale();
  ...
  return <section className="panel"><LeaveGuard dirty={!sent && (busy || Boolean(date) || adults !== '1' || children !== '0' || promotionId !== (params.get('promotion') || '') || optionalIds.length > 0 || name !== (customer.displayName || '') || phone !== (customer.phone || '') || Boolean(allergyStatus) || Boolean(allergies) || accepted)} /><h2>{headingLabel("เลือกวันและส่งคำขอจอง")}</h2><p>{tour.demoCheckoutEnabled ? msg("DEMO · ส่งคำขอแล้วไปที่ทริปของฉัน เพื่อจำลองชำระ ยืนยัน Booking และดูข้อความ LINE โดยไม่ใช้เงินจริง") : msg("ทีมงานจะตรวจสอบที่ว่างก่อนยืนยัน กรุณารอการยืนยันก่อนชำระเงิน")}</p><form noValidate onSubmit={send}>
