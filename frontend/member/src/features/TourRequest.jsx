...
export default function TourRequest({ tour, customer }) {useLocale();
  ...
  return <section className="panel"><LeaveGuard dirty={!sent && Boolean(date)} /><h2>{headingLabel("เลือกวันและส่งคำขอจอง")}</h2><p>{tour.demoCheckoutEnabled ? msg("DEMO · ส่งคำขอแล้วไปที่ทริปของฉัน เพื่อจำลองชำระ ยืนยัน Booking และดูข้อความ LINE โดยไม่ใช้เงินจริง") : msg("ทีมงานจะตรวจสอบที่ว่างก่อนยืนยัน กรุณารอการยืนยันก่อนชำระเงิน")}</p><form noValidate onSubmit={send}>
