...
CI follow-up: the first GitHub Linux run passed installation and npm run check, then found a mobile-auth fixture still navigating to the default port. That fixture now follows GREENVIEW_TEST_ORIGIN as well. Permission validity additionally rejects nonexistent calendar dates and 24:00 rollover; the access tests cover these cases.
