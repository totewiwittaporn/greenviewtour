...
## Customer LINE ID — 2026-09-21

Verified Preview target and migration history: only `20260921120000_customer_line_id` was pending, with no failed migrations. Prisma migrate status returned a blank schema-engine error on this workstation while the maintained TLS-verified pg connection passed. Applied the checked migration SQL and its SHA-256 Prisma history entry atomically through that Preview pool; verified nullable `CustomerProfile.lineId` varchar(100). Generated Prisma Client and restarted local services afterward. No Production change.

A real-database service check saved/read/cleared LINE ID and rejected foreign-account/stale-version access using a disposable transaction rolled back in full. No persistent customer or DEMO records were changed. Other environments should apply the committed migration before regenerating/restarting their API.

## Profile nicknames — 2026-09-21

Verified the same Preview target had only `20260921140000_profile_nicknames` pending. Applied its two additive nullable varchar(50) columns to UserProfile/CustomerProfile with matching SHA-256 Prisma migration history in one transaction through the maintained TLS-verified pool (same workstation schema-engine limitation noted above). Generated the client and restarted local services. A disposable customer transaction verified nickname save/read/trim, omitted-value preservation, clearing and unchanged full name; all fixture data was rolled back. Employee self-profile validation/projection is covered by unit and browser fixtures. Production remains unchanged.
