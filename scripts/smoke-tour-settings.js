...
try{
 ...
 await page.route('**/api/settings/**',route=>{
  ...
  return route.fulfill({json:{rows:result,total:result.length,page:1,pages:1}})
 })
 ...
 console.log(JSON.stringify({result:'PASS',created:Object.keys(rows),checks:['validation','failed save retains values','unsaved discard','view only','independent agent price','search/clear','read retry','keyboard select','mobile overflow','reduced motion'],realDatabaseWrites:0}))
