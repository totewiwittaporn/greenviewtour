...
try {
 ...
 assert.equal(await page.getByLabel('ที่อยู่อีเมล',{exact:true}).inputValue(),'locale@example.invalid')
 assert.equal(await page.getByLabel('รหัสผ่าน',{exact:true}).inputValue(),'unsaved-password-only')
 ...
 await page.getByRole('button',{name:'ไทย',exact:true}).click()
 await page.getByRole('heading',{name:'ลูกค้า · 14 วันข้างหน้า'}).waitFor()
 await page.getByRole('heading',{name:'งานซ่อมบำรุง',exact:true}).waitFor()
