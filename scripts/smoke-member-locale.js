...
try {
 ...
 await page.getByRole('heading',{name:'เข้าสู่ระบบสมาชิก',exact:true}).waitFor()
 await page.getByLabel('อีเมล',{exact:true}).fill('holiday@example.com')
 await page.getByLabel('รหัสผ่าน',{exact:true}).fill('keep-my-password')
 ...
 await page.getByRole('button',{name:'ไทย',exact:true}).click()
 assert.equal(await page.getByLabel('ชื่อผู้ติดต่อ',{exact:true}).inputValue(),'ชื่อที่ยังไม่ส่ง')
 assert.equal(await page.getByLabel('วันเดินทาง',{exact:true}).inputValue(),'2026-09-21')
