...
try {
 ...
 await page.route('**/api/**',route=>{
  ...
  if(path==='/api/me'){me++;return route.fulfill({json:{user:{id:'10000000-0000-4000-8000-000000000001',displayName:'Navigation fixture',roles:[],operations:{booking:true,stock:true},management:{company:true}}}})}
  if(path==='/api/dashboard')return route.fulfill({json:{today:'2026-09-21',scope:'Company',calendar:null,widgets:[]}})
 ...
 await page.goBack();await page.getByRole('heading',{name:'Boat supplies',exact:true}).waitFor();await intact()
 await page.goForward();await page.getByRole('button',{name:'New booking',exact:true}).waitFor();await intact()
 await nav.getByRole('link',{name:'Tour programs & services',exact:true}).click()
 ...
 await page.getByRole('button',{name:'+ Add tour program',exact:true}).waitFor();await intact()
 ...
 await page.getByRole('heading',{name:'Dashboard',exact:true}).waitFor();await intact()
 await page.getByRole('link',{name:'Open my profile →',exact:true}).click()
 await page.getByRole('heading',{name:'Edit profile',exact:true}).waitFor();await intact()
