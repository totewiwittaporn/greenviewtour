...
try {
 ...
 const open=async(entity,singular)=>{await page.goto(`${origin}/${['services','equipment','consumables','stores','components'].includes(entity)?'settings':'operations'}/${entity}`);await page.getByRole('button',{name:`+ Add ${singular}`,exact:true}).click()}
 ...
 await page.setViewportSize({width:390,height:844});await page.emulateMedia({reducedMotion:'reduce'});await page.goto(`${origin}/settings/consumables`);await page.getByRole('button',{name:'Actions for Fixture water',exact:true}).waitFor();assert.equal(await page.getByRole('tabpanel').count(),1);assert.equal(await page.getByRole('tabpanel').getByRole('table').count(),1);assert.equal(await page.getByRole('tab',{name:'Consumables',exact:true}).getAttribute('aria-selected'),'true');assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);await page.screenshot({path:fileURLToPath(new URL('../screenshots.local/operations-mobile.png',import.meta.url)),fullPage:true})
