...
try {
  ...
  await page.route(/\/api\/invitations(?:\?.*)?$/, r => r.fulfill({ json: { invitations: [], roles: [], departments: [], total: 0, page: 1, pageSize: 25, summary: {total:0,awaiting:0,joined:0,inactive:0} } }))
  ...
  assert.equal(await page.locator('.directory-tab-panel[data-active=true] .search-field').evaluate(el => getComputedStyle(el).borderColor), 'rgb(8, 127, 140)')
  ...
  assert.equal(await page.locator('#user-management-panel-users').evaluate(el => el.inert), true)
  await page.screenshot({ path: fileURLToPath(new URL('invitations-aligned-desktop.png', output)), fullPage: true })
  assert.equal(await page.getByRole('searchbox', { name: 'Search invitations by name or email', exact: true }).count(), 1)
  assert.equal(await page.getByRole('table').count(), 1)
  await page.keyboard.press('ArrowLeft')
  assert.equal(await page.getByRole('tab', { name: 'Users', exact: true }).evaluate(el => el === document.activeElement), true)
  ...
  fail = 0
  await page.getByRole('combobox',{name:'Province',exact:true}).click(); await page.getByRole('option',{name:'Krabi · กระบี่',exact:true}).click()
  await page.getByRole('combobox',{name:'District / Amphoe',exact:true}).click(); await page.getByRole('option',{name:'Mueang Krabi · เมืองกระบี่',exact:true}).click()
  await page.getByRole('combobox',{name:'Subdistrict / Tambon',exact:true}).click(); await page.getByRole('option',{name:'Pak Nam · ปากน้ำ',exact:true}).click()
