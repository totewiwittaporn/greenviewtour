...
try {
 ...
 await page.getByRole('menu',{name:'User menu'}).waitFor()
 assert.equal(await page.getByRole('dialog').count(),0)
 assert.equal(await page.getByRole('menu').getByRole('menuitem',{name:'Sign out',exact:true}).count(),1)
 assert.equal(await page.getByRole('menu').getByRole('menuitem',{name:'Open website'}).count(),1)
 await page.screenshot({path:fileURLToPath(new URL('user-info-desktop.png',output)),fullPage:true})
 await page.keyboard.press('Escape')
 await page.getByRole('menu').waitFor({state:'hidden'})
 assert.equal(await page.getByRole('button',{name:'User menu'}).evaluate(el=>el===document.activeElement),true)
