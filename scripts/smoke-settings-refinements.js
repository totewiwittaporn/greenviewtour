...
try{
 ...
 const labels=await page.locator('dialog label').allTextContents();assert.deepEqual(labels.slice(0,7),fields)
 ...
 await page.getByLabel('Subdistrict / Tambon',{exact:true}).fill('กะรน');await page.getByLabel('Postal code',{exact:true}).evaluate(e=>e.value==='83100'||Promise.reject(new Error('Postal code did not follow manual address')));await page.getByLabel('Subdistrict / Tambon',{exact:true}).fill('ราไวย์');await page.getByLabel('House number',{exact:true}).fill('99/1');assert.equal(await page.getByLabel('Postal code',{exact:true}).inputValue(),'83130')
